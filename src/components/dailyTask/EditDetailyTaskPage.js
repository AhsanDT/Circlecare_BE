...
const EditDetailyTaskPage = () => {
    ...
    const [addRequest, { isLoading }] = useUpdateDailyTaskMutation()
    const { data: article } = useGetArticleQuery()
    const [addMedia] = useAddDailyTaskMediaMutation()
    const [detailRequest] = useGetDailyTaskDetailMutation()
    ...
    useEffect(() => {
        if (id) {
            detailRequest(id).unwrap()
                .then((res) => {
                    const data = res?.data[0]
                    setValue('task_type', data?.task_type)
                    setValue('is_all_users', `${data?.is_all_users}`)
                    // setValue('app_user_id', users?.map((item) => {
                    //     if (item?._id?.includes(data?.app_user_id?.value)){
                    //         return {
                    //             value: item?._id,
                    ...
                    //
                    // }))
                    // setValue('app_user_id', data?.app_user_id?.value)
                    ...
                    setValue('other_conditions', data?.other_conditions)
                    setValue('severity_of_symptoms', data?.severity_of_symptoms)
    ...
    console.log('watch', watch('app_user_id'))
...
export default EditDetailyTaskPage;